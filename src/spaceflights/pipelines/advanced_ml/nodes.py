...
def prepare_classification_data(
    ...
    data["price_category"] = pd.cut(
        data["price"],
        bins=[-np.inf, price_quartiles[0.25], price_quartiles[0.5], 
              price_quartiles[0.75], np.inf],
    ...
    logger.info(f"Classification data prepared: {X.shape[0]} samples, {X.shape[1]} features")
...
def prepare_regression_data(
    ...
    logger.info(f"Regression data prepared: {X.shape[0]} samples, {X.shape[1]} features")
...
) -> Tuple[pd.DataFrame, pd.DataFrame, Union[pd.Series, np.ndarray], 
           Union[pd.Series, np.ndarray], KFold]:
...
def train_regression_models(
    ...
    for model_name, config in model_configs.items():
        ...
        logger.info(f"{model_name} - CV RMSE: {np.sqrt(-cv_scores.mean()):.4f} ± {np.sqrt(cv_scores.std()):.4f}")
...
def train_classification_models(
    ...
    for model_name, config in model_configs.items():
        ...
        logger.info(f"{model_name} - Best params: {grid_search.best_params_}")
        logger.info(f"{model_name} - CV F1: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}")
...
def create_model_comparison_report(
    ...
    logger.info(f"Best regression model: {report['best_models'].get('regression', {}).get('model', 'N/A')}")
    logger.info(f"Best classification model: {report['best_models'].get('classification', {}).get('model', 'N/A')}")
